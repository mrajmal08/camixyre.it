    ...
    var monthlySalesChart = document.getElementById('monthly-sales-chart').getContext('2d');
      new Chart(monthlySalesChart, {
        ...
        data: {
          ...
          datasets: [{
            label: 'Sales',
            data: [{{ count($january) }},{{ count($february) }},{{ count($march) }},{{ count($april) }},{{ count($may) }},{{ count($june) }},{{ count($july) }},{{ count($august) }},{{ count($september) }},{{ count($octobor) }},{{ count($november) }},{{ count($december) }}],
...
$(function() {
  ...
  function generateDayWiseTimeSeries(s, count) {
    var values = [[
      @foreach ($last7DaysAnalytics as $visitors)
        {{ $visitors['visitors'] }},
      @endforeach
    ], [
      @foreach ($last7DaysAnalytics as $pageViews)
        {{ $pageViews['pageViews'] }},
      @endforeach
    ]];
    var i = 0;
    var series = [];
    var x = new Date("{{ substr($last7DaysAnalytics[0]['date'], 0, -9) }}").getTime();
    while (i < count) {
  ...
  // @foreach ($countryVisits['rows'] as $countries) {{ strval(str_replace(" ", "", $countries[0])) . "," }} @endforeach
  ...
  if($('#chartjsDoughnut').length) {
    new Chart($('#chartjsDoughnut'), {
      type: 'doughnut',
      data: {
        labels: [<?php foreach($countryVisits['rows'] as $countries): $label = strval(str_replace(" ", "", $countries[0])); echo("'$label'" . ",");  endforeach; ?>],
        datasets: [
          {
            label: "Population (millions)",
            backgroundColor: ["#7ee5e5","#f77eb9","#4d8af0","#636cd6","#ff3366"],
            data: [@foreach ($countryVisits['rows'] as $countries) {{ str_replace(" ", "", $countries[1]) . "," }} @endforeach]
          }
        ]
      }
    });
